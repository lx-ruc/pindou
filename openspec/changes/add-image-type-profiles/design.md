...
**§4.1 结论（已收口，2026-07 实测）**：
- logo（真·平涂卡通）5 色，最小成对 Oklab 距离 **14.06**；spatial 阈值 6–20 全程色数不变（无损）。
- 合成相邻近色对：距离 5.5 / 8.2 在 threshold=10 被吞并；≥14.5 保留 ⇒ **spatial 阈值 T 会合并任何 Oklab 距离 <T 的相邻意图色**。
- 渐变（照片连续色调代理）：基线 13 色 → spatial@10 = 6、@15 = 4（平滑生效）。

⇒ spatial 对**平涂卡通**无损（意图色距通常 ≥14），但对**带阴影/渐变的卡通**（意图近色 <10）有损（抹平层次）。故保守默认 **cartoon `spatialEnabled=false` 正确**（保护风险子类；平涂卡通本就不需要 spatial），**photo `spatialEnabled=true` 正确**（平滑连续色调）。**SD-3 步开关确属必要**（让带阴影卡通能关 spatial）。当前占位默认经实测确认、无需调整；边界经验（阈值 = 会被吞并的最小意图色距）作为用户自定义时的判据。
