...
export function drawGrid(
  ...
  const M = withZones ? Math.round(bp * 1.7) : 0
  const ox = M
  const oy = M

  g.fillStyle = '#ECE4D2'
  g.fillRect(ox, oy, cols * bp, rows * bp)
  ...
  for (let r = 0; r < rows; r++) {
    for (let c = 0; c < cols; c++) {
      ...
      const y = oy + r * bp
      g.fillStyle = hex
      g.fillRect(x + 0.5, y + 0.5, bp - 1, bp - 1)
  ...
  if (withZones) {
    ...
    g.lineWidth = Math.max(2, bp * 0.14)
    g.lineCap = 'square'
    g.beginPath()
    for (let i = 0; i <= cols; i += ZONE) {
      const x = Math.round(ox + i * bp) + 0.5
      g.moveTo(x, oy)
      g.lineTo(x, oy + rows * bp)
    }
    for (let i = 0; i <= rows; i += ZONE) {
      const y = Math.round(oy + i * bp) + 0.5
      g.moveTo(ox, y)
      g.lineTo(ox + cols * bp, y)
    }
    g.stroke()
    g.lineWidth = Math.max(2.5, bp * 0.16)
    g.strokeRect(ox + 0.5, oy + 0.5, cols * bp - 1, rows * bp - 1)

    g.fillStyle = '#23202E'
    g.font = '700 ' + Math.floor(bp * 0.8) + 'px sans-serif'
    g.textBaseline = 'middle'
    for (let zc = 0; zc * ZONE < cols; zc++) {
      g.textAlign = 'center'
      g.fillText(String.fromCharCode(65 + zc), ox + (zc * ZONE + ZONE / 2) * bp, oy - M * 0.5)
    }
    for (let zr = 0; zr * ZONE < rows; zr++) {
      g.textAlign = 'center'
      g.fillText(String(zr + 1), ox - M * 0.5, oy + (zr * ZONE + ZONE / 2) * bp)
    }
  }
}
...
export function drawProgressOverlay(
  ...
  const M = showZones ? Math.round(bp * 1.7) : 0
  const ox = M
  const oy = M
  ...
  for (let r = 0; r < rows; r++) {
    for (let c = 0; c < cols; c++) {
      ...
      g.fillRect(x, y, bp, bp)
      g.strokeStyle = '#fff'
      g.lineWidth = Math.max(1.6, bp * 0.16)
      g.lineCap = 'round'
      g.lineJoin = 'round'
      const cx = x + bp / 2
      const cy = y + bp / 2
  ...
  if (guide) {
    for (let k = 0; k < 3 && nextIdx + k < routeOrder.length; k++) {
      ...
      g.lineWidth = k === 0 ? Math.max(2.5, bp * 0.2) : Math.max(1.5, bp * 0.1)
      g.globalAlpha = k === 0 ? 1 : 0.4
      g.strokeRect(x + 1, y + 1, bp - 2, bp - 2)
    }
    g.globalAlpha = 1
  }
}
...
export function drawComposed(
  ...
  drawGrid(g, hexGrid, rows, cols, bp, true, true, brand)
  g.restore()
